...
namespace BreachScenarioEngine.Mcp.Editor
{
    ...
    private static (bool Success, string Message) Execute(string command, string raw)
    {
        try
        {
            return command switch
            {
                ...
                "manage_mission" => ManageMission(raw),
    ...
    private static (bool, string) ManageMission(string raw)
    {
        var action = (JsonArgumentString(raw, "action") ?? "validate_template").ToLowerInvariant();
        return MissionPipelineEditorService.Execute(action, raw);
    }
