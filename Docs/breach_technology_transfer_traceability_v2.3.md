...
| Visibility graph | `Day1LevelGenerator` tracks room-to-room visibility through doors and open lines of sight | `Assets/Scripts/Generation/TacticalGraphs/VisibilityGraphBuilder.cs` | Deterministic portal-to-portal visibility edges with openness weighting | Scene raycast or editor-only visibility checks during Step 6 | `VisibilityGraph.edges` count and `visibilityRayCount` in `verification_summary.json` |
| Hearing graph | `Day1LevelGenerator` tracks acoustic adjacency and attenuation through portals | `Assets/Scripts/Generation/TacticalGraphs/HearingGraphBuilder.cs` | Deterministic portal-to-portal hearing edges with wall and door attenuation | Runtime physics audio simulation during Step 6 | `HearingGraph.edges` count and `hearingOverlapPercentage` in `verification_summary.json` |
